...
# Transfer log produced by rsync
RSLOGDIR="Rsync_logs"

tput bold
echo "Creating log dir $RSLOGDIR (will overwrite existing)"
tput sgr0
[ -d ${RSLOGDIR} ] && rm -rf ${RSLOGDIR}
mkdir -p ${RSLOGDIR}
...
for FILE in ${FILE_LIST[@]}
do
    B_NAME=$(basename "${FILE}" ."{$TYPE}")
    #echo "Adding $B_NAME to transfer list"
    LOG_FILE="${RSLOGDIR}/${B_NAME}_log.txt"
    echo "rsync ${FLAGS} --log-file=${LOG_FILE} ${FILE} ${DEST}" >> $JL_FILE
    ((COUNT++))
done
...
PLOG="parallel_log.txt"
parallel --progress --jobs ${CPU} --joblog ${PLOG} < ${JL_FILE}
tput bold
echo -ne "\nDone! - Run logged to: $PLOT and Rsync logs are in: $RSLOGDIR\n"
tput sgr0
